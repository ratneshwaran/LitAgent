...
def search_openalex(query: str, filters: SearchFilters) -> List[Paper]:
    ...
    papers = []
    per_page = min(50, config.search_max_per_source)
    ...
    if filters.start_year:
        search_params["filter"].append(f"from_publication_date:{filters.start_year}")
    if filters.end_year:
        search_params["filter"].append(f"to_publication_date:{filters.end_year}")
    
    # Add venue filter if specified
    if filters.venues:
        venue_filter = "|".join(filters.venues)
        search_params["filter"].append(f"primary_location.source.display_name:{venue_filter}")
    
    # Add abstract requirement
    search_params["filter"].append("has_abstract:true")
